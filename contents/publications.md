
- **A Nonblocking Multistage Switching Network for Distributed Quantum Computing**  
  Yu Liu, **Yingling Mao**, Xu Xu, Xiaojun Shang, Fan Ye, and Yuanyuan Yang  
  *IEEE Transactions on Networking (TON)*, to appear (accepted for publication), 2025  

- **Remote Gate Scheduling in Distributed Quantum Computing**  
  Xu Xu, Yu Liu, **Yingling Mao**, and Yuanyuan Yang  
  *IEEE 44th International Conference on Distributed Computing Systems (ICDCS)*, to appear (accepted for publication), 2025  

- **Provable Approximation Algorithms for Online Traffic-sensitive SFC Deployment**  
  **Yingling Mao**, Xiaojun Shang, and Yuanyuan Yang  
  *IEEE Transactions on Networking (TON)*, to appear (accepted for publication), 2025  

- **Network Topology Design for Distributed Quantum Computing**  
  **Yingling Mao**, Yu Liu, Xiaojun Shang, and Yuanyuan Yang  
  *IEEE 44th International Conference on Distributed Computing Systems (ICDCS)*, pp. 1213–1223, 2024  

- **Performance Analysis of Interconnection Networks for Distributed Quantum Computing**  
  **Yingling Mao**, Yu Liu, Xu Xu, Xiaojun Shang, and Yuanyuan Yang  
  *IEEE Global Communications Conference (GLOBECOM)*, 2024  

- **Qubit Allocation for Distributed Quantum Computing**  
  **Yingling Mao**, Yu Liu, and Yuanyuan Yang  
  *IEEE Conference on Computer Communications (INFOCOM)*, pp. 1–10, 2023  

- **Probability-aware Qubit-to-Processor Mapping in Distributed Quantum Computing**  
  **Yingling Mao**, Yu Liu, and Yuanyuan Yang  
  *Workshop on Quantum Networks and Distributed Quantum Computing (QuNet)*, pp. 51–56, 2023 

- **Joint Virtual Network Function Placement and Flow Routing in Edge-Cloud Continuum**  
  **Yingling Mao**, Xiaojun Shang, Yu Liu, and Yuanyuan Yang  
  *IEEE Transactions on Computers (TC)*, vol. 73, no. 3, pp. 872–886, Dec. 2023  

- **Joint Task Offloading and Resource Allocation in Heterogeneous Edge Environments**  
  Yu Liu, **Yingling Mao**, Zhenhua Liu, Fan Ye, and Yuanyuan Yang  
  *IEEE Transactions on Mobile Computing (TMC)*, vol. 23, no. 6, pp. 7318–7334, Nov. 2023  

- **Deep Learning-Assisted Online Task Offloading for Latency Minimization in Heterogeneous Mobile Edge**  
  Yu Liu, **Yingling Mao**, Zhenhua Liu, and Yuanyuan Yang  
  *IEEE Transactions on Mobile Computing (TMC)*, vol. 23, no. 5, pp. 4062–4075, Jun. 2023  

- **Availability Aware Online Virtual Network Function Backup in Edge Environments**  
  Yu Liu, Xiaojun Shang, **Yingling Mao**, Zhenhua Liu, and Yuanyuan Yang  
  *IEEE Transactions on Mobile Computing (TMC)*, vol. 23, no. 5, pp. 3909–3922, Jun. 2023  

- **Ant Colony based Online Learning Algorithm for Service Function Chain Deployment**  
  **Yingling Mao**, Xiaojun Shang, and Yuanyuan Yang  
  *IEEE Conference on Computer Communications (INFOCOM)*, pp. 1–10, 2023  

- **Joint Task Offloading and Resource Allocation in Heterogeneous Edge Environments**  
  Yu Liu, **Yingling Mao**, Xiaojun Shang, Zhenhua Liu, Fan Ye, and Yuanyuan Yang  
  *IEEE Conference on Computer Communications (INFOCOM)*, pp. 1–10, 2023  

- **Online Container Scheduling for Data-intensive Applications in Serverless Edge Computing**  
  Xiaojun Shang, **Yingling Mao**, Yu Liu, Yaodong Huang, Zhenhua Liu, and Yuanyuan Yang  
  *IEEE Conference on Computer Communications (INFOCOM)*, pp. 1–10, 2023  

- **Energy-aware Online Task Offloading and Resource Allocation for Mobile Edge Computing**  
  Yu Liu, **Yingling Mao**, Xiaojun Shang, Zhenhua Liu, and Yuanyuan Yang  
  *IEEE 43rd International Conference on Distributed Computing Systems (ICDCS)*, pp. 339–349, 2023  

- **Joint Resource Management and Flow Scheduling for SFC Deployment in Hybrid Edge-and-Cloud Network**  
  **Yingling Mao**, Xiaojun Shang, and Yuanyuan Yang  
  *IEEE Conference on Computer Communications (INFOCOM)*, pp. 170–179, 2022  

- **Provably Efficient Algorithms for Traffic-sensitive SFC Placement and Flow Routing**  
  **Yingling Mao**, Xiaojun Shang, and Yuanyuan Yang  
  *IEEE Conference on Computer Communications (INFOCOM)*, pp. 950–959, 2022  

- **Distributed Cooperative Caching in Unreliable Edge Environments**  
  Yu Liu, **Yingling Mao**, Xiaojun Shang, Zhenhua Liu, and Yuanyuan Yang  
  *IEEE Conference on Computer Communications (INFOCOM)*, pp. 1049–1058, 2022  

- **Enabling QoE Support for Interactive Applications over Mobile Edge with High User Mobility**  
  Xiaojun Shang, Yaodong Huang, **Yingling Mao**, Zhenhua Liu, and Yuanyuan Yang  
  *IEEE Conference on Computer Communications (INFOCOM)*, pp. 1289–1298, 2022  

- **Near-Optimal Resource Allocation and Virtual Network Function Placement at Network Edges**  
  **Yingling Mao**, Xiaojun Shang, and Yuanyuan Yang  
  *IEEE 27th International Conference on Parallel and Distributed Systems (ICPADS)*, pp. 18–25, 2021  

- **Greening Reliability of Virtual Network Functions via Online Optimization**  
  Xiaojun Shang, Yu Liu, **Yingling Mao**, Zhenhua Liu, and Yuanyuan Yang  
  *IEEE/ACM 28th International Symposium on Quality of Service (IWQoS)*, pp. 1–10, 2020  

- **Scalability of Wireless Fingerprinting based Indoor Localization Systems**  
  **Yingling Mao**, Ke Liu, Hao Li, Xiaohua Tian, and Xinbing Wang  
  *IEEE International Conference on Sensing, Communication, and Networking (SECON)*, pp. 1–9, 2018 [[Paper]](https://doi.org/10.1109/LCOMM.2021.3081593)

